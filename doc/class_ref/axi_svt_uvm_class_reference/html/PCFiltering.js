window.onload = function() {
        ...
        var script_path=script_element[0].src;
        d=script_path.split("/html/");
        opdir=d[0]+"/";
        var links1 = document.getElementsByTagName("a");
        var links2 = document.getElementsByTagName("area");
        update_href(links1,opdir);
        update_href(links2,opdir);
}
function update_href(links,opdir){
        var url_string,curr_path,new_tab_flag;
        for (var i = 0; i < links.length; i++) {
                new_tab_flag=0;
                var attrType='href';
                url_string = curr_path = links[i].getAttribute("href");
                if (!curr_path) {
                    url_string = curr_path = links[i].getAttribute('xlink:href');
                    attrType = 'xlink:href'
                }
                if(curr_path){
                        var dir_struct=curr_path.split("../../");
                        if(dir_struct.length==1){
                                var dir_struct1=curr_path.split("./");
                                if(dir_struct1.length==1){
                                        if(dir_struct1[0].indexOf("#")==0)
                                                url_string=curr_path;
                                        else if(dir_struct1[0].indexOf("source.html")!=-1 && dir_struct1[0].indexOf("source/")==-1 )
                                                url_string=opdir+"html/source/"+dir_struct1;
                                        else
                                                url_string=opdir+"html/"+dir_struct1;
                                }
                                else if(dir_struct1.length==2){
                                        var temp=dir_struct1[1].split("/");
                                        if(temp.length==1){
                                                url_string=curr_path;
                                        }
                                        else if(temp.length==2){
                                                url_string=opdir+"html/"+temp[0]+"/"+temp[1];
                                        }
                                        else if(temp.length==3){
                                                url_string=opdir+"html/"+temp[1]+"/"+temp[2];
                                        }
                                }
                        }
                        else if(dir_struct.length==2){
                                url_string=opdir+"../"+dir_struct[1];
                                new_tab_flag=1;
                        }
                        if(url_string != undefined)
                                links[i].setAttribute(attrType, url_string);
                        if(new_tab_flag)
